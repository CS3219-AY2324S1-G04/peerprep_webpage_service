...
import { Box, Button, Dropdown, Chip as JoyChip, Menu, MenuButton, MenuItem, Typography, useTheme } from "@mui/joy"
...
export default function ProblemsTable() {
  ...
  const { items, sorting, paging, filtering } = Table.useTable(
    questionsList,
    {
      sortKey: "title",
  ...
  const getCategoriesToDisplay = (categories: string[]) => {
    ...
    const remainingCatgoriesCount = categories.length - categoriesToDisplay.length
  ...
  const onClickComplexityFilter = (complexityValue: string) => {
    ...
    filtering.setFilterColumns(new Map(filtering.filterColumns.set("complexity", filterValues)))

    const updatedFilters = filters
    const currentFilterIdx = filters.findIndex((f) => f.columnFilterKey === "complexity")
    if (currentFilterIdx !== -1) {
      updatedFilters[currentFilterIdx] = { columnFilterKey: "complexity", value: complexityValue }
    } else {
      updatedFilters.push({ columnFilterKey: "complexity", value: complexityValue })
    }
  ...
  const onClickCategoryFilter = (category: string) => {
    ...
    if (!filterValues.includes(category)) {
      ...
      filtering.setFilterColumns(new Map(filtering.filterColumns.set("categories", filterValues)))
  ...
  const onDeleteFilterChip = (filterObj: Filter) => {
    const existingFilterValues = filtering.filterColumns.get(filterObj.columnFilterKey) ?? []
    const updatedFilterValues = existingFilterValues.filter((value) => value !== filterObj.value)

    filtering.setFilterColumns(new Map(filtering.filterColumns.set(filterObj.columnFilterKey, updatedFilterValues)))
    setFilters(filters.filter((f) => !(f.columnFilterKey === filterObj.columnFilterKey && f.value === filterObj.value)))
  }
  ...
  return (
    <div>
          ...
          <Dropdown>
            ...
            <Menu sx={styles.complexityMenu}>
              {Object.keys(QuestionComplexity).map((complexityValue) =>
                <MenuItem key={complexityValue} onClick={() => onClickComplexityFilter(complexityValue)}>
                  {complexityValue}
                </MenuItem>
              )}
            </Menu>
      ...
      >
        <Box sx={styles.filtersBox}>
          {
            filters.map((filterObj) =>
              <Chip
                ...
                endDecorator={<ChipDelete onClick={() => onDeleteFilterChip(filterObj)} />}
              >
                {filterObj.value}
              </Chip>
            )
        ...
        <Button size="sm" variant="plain" startDecorator={<ReplayIcon />} onClick={() => {
          setFilters([])
      ...
      <Table>
        <Table.Header {...sorting}>
          <Table.ColumnHead id="title" cellProps={{ style: styles.widerColumnHead }}>Title</Table.ColumnHead>
          <Table.ColumnHead sortable={false} id="categories" cellProps={{ style: styles.widerColumnHead }}>Categories</Table.ColumnHead>
            ...
            <Table.Row key={item._id}>
              ...
              <Table.Cell>
                <Box sx={styles.problemCategoriesBox}>
                  {
                    getCategoriesToDisplay(item.categories).map((category: string) => {
                      const isExtraCategoriesCount = category.substring(0, 1) === '+'
                      const ChipComponent = isExtraCategoriesCount ? JoyChip : Chip
                      return (
                        <ChipComponent key={category}>{category}</ChipComponent>
                      )
                    })
                  }
                </Box>
